...
-- 연습문제 01 : 2000에서 3000사이의 월급을 받는 사원을 조회해보시오.
SELECT * FROM employees WHERE salary >= 2000 AND salary <= 3000;
SELECT * FROM employees WHERE salary BETWEEN 2000 AND 3000;

-- 연습문제 02 : 부서번호가 30번이거나 60번이거나 90번인 사원들의 모든 정보를 조회해보시오.
SELECT 
    * 
FROM  
    employees 
...
-- 연습문제 03 : 사원번호가 짝수인 사원들만 조회해보시오. 
SELECT * FROM employees WHERE mod(employee_id, 2) = 0;

-- COLUMN BETWEEN A AND B : 해당 컬럼의 값이 A 이상  B이하일 때 True
SELECT * FROM employees WHERE salary BETWEEN 2000 AND 3000;
...
-- 값이 null인 조건을 만들때는 IS NULL을 이용해야 한다.
...
-- 연습문제 04 : LAST_NAME의 뒤에서 세 번째 글자가 a인 모든 사원을 조회해보시오.
SELECT * FROM employees WHERE last_name LIKE '%a___';

-- 연습문제 05 : LAST_NAME의 3번째 글자가 r이고 핸드폰 번호에 .이 3개 포함된 모든 사원을 조회해보시오.
SELECT * FROM employees WHERE last_name LIKE '__r%' AND phone_number LIKE '%.%.%.%';