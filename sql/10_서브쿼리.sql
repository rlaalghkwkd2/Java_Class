...
/*
        # 서브쿼리
            ...
            - 서브 쿼리를 포합하고 있는 쿼리를 메인 쿼리라고 한다.
            - 서브 쿼리로 비교연산을 하는 경우 반드시 연산자의 오른쪽에 기술해야 한다.
            - 반드시 괄호로 둘러쌓아야 한다.
            -    서브 쿼리는 메인 쿼리가 실행되기 전에 한번만 실행 된다.
            
            # 단일 행 서브 쿼리
                - 서브 쿼리의 수행 결과가 단 하나의 행만 반환하는 서브 쿼리
                - =, >, >=, <, <=, <> 하나의 값만 요구하는 비교 연산자 이므로 단일 행 서브 쿼리와 함께 사용해야 한다.
*/
...
SELECT * FROM employees where department_id = (
    ...
    FROM
      employees
     WHERE
      last_name = 'King' AND first_name LIKE 'S%'
); -- 단일 행 연산자는 서브 쿼리의 결과가 단일 행이 아닌 경우 에러가 발생한다.

--연습문제01: 서브 쿼리를 이용해 Ki Gee와 동일안 직급을 가진 사원을 조회해보세요
SELECT * FROM
    employees 
WHERE 
    job_id  =(
...
FROM
    employees
 WHERE
    first_name ='Ki' AND last_name = 'Gee'
);
--연습문제02: 서브 쿼리를 이용해 Ki Gee보다 급여를 많이 받는 사원들의 이름과 급여를 조회
SELECT
    first_name,
    last_name,
    salary
...
--연습문제03: 서브 쿼리를 이용해 Purchasing부서에서 근무하는 사원의 이름과 부서번호를 조회해 보세요
SELECT
    first_name,
    last_name,
   department_id
...
WHERE
    ...
    WHERE
        salary >= 10000)
ORDER BY -- (20 , 30, 70, 80 ,90, 100, 110) : 다중 행 서브 쿼리는 다중 행 연산자와 함께 사용해야 한다.
    department_id ASC;
...
-- => 50번 부서에서 제일 많이 받는 사원보다 더 많이 받는 사원들으 ㄹ조회
SELECT * FROM employees WHERE salary > ALL (SELECT salary FROM employees WHERE department_id = 100);
    
 SELECT * FROM employees WHERE salary >  (SELECT MIN(salary) FROM employees WHERE department_id = 100); 
 
 --연습문제04: 프로그래머들 보다 급여를 많이 받는 사원들의 이름/급여/직책ID를 조회 하되 프로그래머는 출력하지말아보세요
...
--연습문제05: 가장 적게 받는 프로그래머보다 많이 받는 모든 사원을 조회
...
-- 같은 내용 이지만 다른 형태로 꺼내올 수 있다.
SELECT * FROM employees WHERE EXISTS(SELECT 1 FROM job_history WHERE employees.employee_id = job_history.employee_id);  
