...
-- 연습문제02: empp 테이블과 depp 테이블의 기본키/외래키 제약 조건을 설정해보세요  9(기본키가 아닌것을 외래키로 참조할 수 없다)
desc employees;
desc empp;
...
ALTER TABLE empp add CONSTRAINT empp_eid_pk PRIMARY KEY (employee_id);
ALTER TABLE depp add CONSTRAINT depp_did_pk PRIMARY KEY (department_id);
ALTER TABLE jobb add CONSTRAINT jobb_id_pk  PRIMARY KEY (job_id);


ALTER TABLE empp add ( -- jobs을 추가한거
...
/*
        ALTER TABLE empp MODIFY(
                ...
                job_id      CONSTRAINT empp_job_id_nn NOT NULL,
...
ALTER TABLE empp add  
        CONSTRAINT empp_dept_fk FOREIGN KEY (department_id) REFERENCES depp (department_id) ON DELETE CASCADE;
...
ALTER TABLE empp drop CONSTRAINT empp_dept_fk;
ALTER TABLE empp add  
        CONSTRAINT empp_dept_fk FOREIGN KEY (department_id) REFERENCES depp (department_id) ON DELETE SET NULL;
...
-- 2. 기본값인 상태로 50번 부서를 삭제
 ...
 SELECT * FROM empp ORDER BY department_id DESC;
 -- ※ ON DELETE절이 설정되어 있지 않은 경우에는 사직 레코드가 있는 부모키를 삭제 할 수 없다
