...
-- 연습문제 01 : 가장 최근에 입사한 사원의 입사일과 
--                  입사한지 가장 오래된 사원의 입사일을 출력하는 쿼리문을 작성해보시오.
...
/*
    ...
    - GROUP으로 나눈 경우에는 그룹을 나누는 기준이되는 컬럼과 그룹 함수의 결과만 컬럼으로 선택할 수 있다.
*/

SELECT COUNT(*), job_id FROM employees GROUP BY job_id;
SELECT COUNT(*), department_id FROM employees GROUP BY department_id;

-- 연습문제02 : 작 직책별 평균 연봉을 구해보세요(※ 월급 아님 , 커미션 적용)


-- 카페, 병원, 음식점, 쇼핑몰, 편의점.. 들을 운영하는데에 필요한 프로그램
...
SELECT 
    job_id,AVG(salary *(NVL(commission_pct,0) + 1) * 12)  AS "평균 연봉" FROM employees GROUP BY job_id ;
...
-- 연습 문제03 : 부서별 최대 급여와, 최소 급여를 출력하되 최대 급여가 5000이상인 부서만 출력
SELECT
MIN(salary),
MAX(salary)
FROM 
...
-- 연습문제04 :  부서별 평균 급여를 출력하되 해당 부서에 소속된 사원이 5명 이상인 경우에만 출력
SELECT
    ROUND(AVG(salary)),
    department_id
FROM 
