...
-- ORDER BY절을 통해 조회한 내용을 원하는 컬럼 기준으로 정렬할 수 있다.

-- ORDER BY
/* 
    -  ORDER  BY 절을 통해 조회한 내용을 원하는 컬럼 기준으로 정렬할 수 있다.
    -  ORDER  BY 컬럼명 ASC : ACSENDING, 오름차순, 생략가능
...
SELECT * FROM employees ORDER BY salary;  -- 월급 기준 오름차 순
SELECT * FROM employees ORDER BY salary DESC;  -- 월급 기준 내림차 순

-- # 정렬 조건을 여러개 줄 수 있다
-- 먼저 직급순으로 정렬한 후 , LAST_NAME 알파벳 순으로 정렬한 후 , 사번으로 정렬
